...
static int writer_function(void *data) {
    ...
    while(counter < MAX_COUNT) {
        ...
        if(counter == MAX_COUNT - 1) {
            ...
            printk("RW-semaphore linked list insert: %lluns\n", delay);
    ...
    while(counter < MAX_COUNT * 2) {
        down_read(&counter_lock);

        if(current_node->data == finding_num || counter == MAX_COUNT * 2 - 1) {
            getnstimeofday(&spclock[1]);

            delay = calclock(spclock);
            printk("RW-semaphore linked list search: %lluns\n", delay);
            counter = MAX_COUNT * 2;

            current_node = list_first_entry(&my_list, struct my_node, list);
            getnstimeofday(&spclock[0]);
            up_read(&counter_lock);
            break;
        } else {
            counter++;
        }
	...
	kfree(tmp);
        ...
        if(counter == MAX_COUNT * 3 - 1) {
            ...
            delay = calclock(spclock);
            printk("RW-semaphore linked list remove: %lluns\n", delay);
        }
...
static int __init semaphore_mod_init(void) {
...
static void __exit semaphore_mod_exit(void) {
    printk("%s, Exiting module\n", __func__);
}

module_init(semaphore_mod_init);
module_exit(semaphore_mod_exit);
